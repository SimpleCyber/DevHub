    ...
    if (userId) {
      try {
        if (activeTabId.startsWith("new-")) {
          ...
          setTabs((prev) =>
            prev.map((t) =>
              t.id === activeTabId ? { ...t, id: savedPath.id } : t,
            ),
          );
      ...
      >
        <div className="career-path-container flex flex-col h-screen">
          ...
          <div className="flex-1 relative overflow-hidden">
            <div
              className={`career-map-wrapper ${chatOpen ? "chat-open" : ""}`}
            >
              <CareerMap
            ...
            <div className={`career-chat-wrapper ${chatOpen ? "open" : ""}`}>
              ...
              >
                <ChevronRight
                  className={`w-6 h-6 text-gray-500 hover:text-gray-800 transition-transform ${chatOpen ? "" : "rotate-180"}`}
                />
              </button>
              <CareerChat
                messages={activeTab?.messages || []}
